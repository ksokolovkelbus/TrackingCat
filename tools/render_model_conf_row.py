...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Render one model/imgsz at several confidence thresholds separately, then stitch horizontally.")
    p.add_argument("video")
    p.add_argument("--model", default="yolo26s.pt")
    p.add_argument("--label", default="YOLO26s")
    p.add_argument("--imgsz", type=int, default=384)
    p.add_argument("--confs", nargs="+", type=float, default=[0.05, 0.07, 0.09])
...
def render_one(args, video: Path, fps: float, conf: float, out: Path, logger) -> int:
    ...
    title = f"{args.label} | imgsz={args.imgsz} | conf={conf:.2f}"
    cap = cv2.VideoCapture(str(video))
    writer = None
    processed = 0
    source_frame = 0
    ...
    try:
        while True:
            ...
            panel = cv2.resize(panel, (args.panel_width, args.panel_height), interpolation=cv2.INTER_AREA)
            if writer is None:
...
def stitch(parts: list[Path], output: Path, fps: float, stride: int, show: bool) -> int:
    ...
    try:
        while True:
            ...
            frames += 1
            if show:
                cv2.imshow("model confidence row", row)
...
def main() -> int:
    ...
    conf_label = "_".join(f"{c:.2f}".replace(".", "p") for c in args.confs)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output = Path(args.output) if args.output else Path("recordings") / f"{stem}_imgsz{args.imgsz}_conf_{conf_label}_{stamp}.mp4"
    tmp = output.parent / f".{output.stem}_parts"
    parts = []
    for i, conf in enumerate(args.confs, start=1):
        part = tmp / f"conf_{conf:.2f}.mp4"
        print(f"Pass {i}/{len(args.confs)+1}: render {args.label} imgsz={args.imgsz} conf={conf:.2f}")
        render_one(args, video, fps, conf, part, logger)
        parts.append(part)
    print(f"Pass {len(args.confs)+1}/{len(args.confs)+1}: stitch")
